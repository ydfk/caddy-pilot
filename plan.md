...
- [x] Caddy 二进制、当前版本与活动配置统一持久化到 `/data`，原生数据与日志在后续日志阶段收口。
- [x] 修复慢速下载超时，增加可轮询的更新任务状态与失败原因。
- [x] 支持上传 Caddy 可执行文件、ZIP 和 tar.gz，并校验平台、版本及阿里云 DNS 模块。
- [x] 启动恢复活动配置，发布与回滚原子持久化，避免站点配置在重启后丢失。
- [x] 动态比较业务配置、持久化配置与 Caddy 当前 JSON，识别运行版本及配置漂移。
- [x] 合并配置发布与配置版本界面，并提供漂移修复操作。
- [x] 修复新增 DNS Provider 导致证书草稿丢失，所有普通弹窗禁止点击遮罩关闭。
- [x] 统一 API 错误解析和表单内错误展示，覆盖证书、DNS、密码本、站点、更新、发布与回滚。
- [x] 通配符证书允许多个域名且共用一个 DNS Provider，下拉仅显示配置名称。
- [x] 移除无效 WebSocket 开关，保留旧字段兼容并固定新表单为 false。
- [x] 从 Caddy PEM 证书读取域名、签发时间、到期时间、颁发机构、序列号与有效状态。
- [x] 增加日志菜单，在线轮询查看系统日志与 Caddy 日志，日志文件写入 `/data/logs` 并轮转。
- [x] 完成后端、前端、Action、单镜像 Docker 与浏览器全量验收。
...
- [x] 默认更新源迁移到 CaddyPilot Release manifest，下载支持断点续传、重试、强制 SHA-512 校验与详细失败信息。
- [x] 更新任务持久化，页面刷新后恢复进度或最后失败原因，并收敛 Caddy 运行时版本操作布局。
- [x] 增加脱敏的 DNS Provider 业务调用日志，并在日志页面提供独立标签。
- [x] 证书状态综合站点引用、运行配置、PEM 和 Caddy 错误，区分未引用、等待发布、签发中、失败、已签发、即将到期和过期。
- [x] 配置发布、同步状态与版本记录真正合并为单一配置管理卡片。
- [x] 日志按新到旧返回并在前端顶部插入，确保最新记录始终位于顶部。
- [x] 增加只读 Caddyfile 生成、托管 Caddy 语法验证、版本留存与导出，JSON 继续作为发布和回滚真源。
- [ ] 完成 Go、前端、Docker、Release 工作流与浏览器全量验收，确认不暴露 2019。
